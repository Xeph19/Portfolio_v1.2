...
const Navbar = ({ className = "" }) => {
  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-1/2 -translate-x-1/2 z-50 rounded-full w-[90%] max-w-5xl transition-all duration-500 ${
        scrolled
    ...
    >
      <div className="px-8 py-3 flex items-center justify-between w-full font-poppins">
        {/* Logo / Title */}
        <div
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            className="absolute top-full right-0 mt-4 w-56 bg-white/95 backdrop-blur-md rounded-2xl shadow-lg flex flex-col gap-2 p-4 text-gray-700 font-medium md:hidden"
          >
            {navLinks.map((link) => (
              <button
                key={link.name}
                className={`px-4 py-2 rounded-md text-left w-full transition-all ${
                  active === link.id
